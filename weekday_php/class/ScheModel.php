 ...
 */
class ScheModel extends BaseModel {
    //----------------------------------------------------
    // 送信するテキストの移動処理
    //----------------------------------------------------
    public function move_text($id,$messe_No,$text){
        try {
            $this->pdo->beginTransaction();
            $sql = "INSERT  INTO sche (id,messe_No,send_text)
            		VALUES ( :id, :messe_No, :send_text)";
            $stmh = $this->pdo->prepare($sql);
            $stmh->bindValue(':id',   $id,   PDO::PARAM_INT );
            $stmh->bindValue(':messe_No',   $messe_No,   PDO::PARAM_INT );
            $stmh->bindValue(':send_text',  $text,  PDO::PARAM_STR );
            $stmh->execute();
            $this->pdo->commit();
    ...
    //----------------------------------------------------
    public function check_sche(){
    	try {
    		$sql= "SELECT * FROM sche";
    		$stmh = $this->pdo->prepare($sql);
    		$stmh->execute();
    		$count = $stmh->rowCount();
    	} catch (PDOException $Exception) {
    ...
    //----------------------------------------------------
    // ken_messe1のsendday入力処理
    //----------------------------------------------------
    public function input_sendday($id,$messe_no,$sendday){
    	try {
    		$this->pdo->beginTransaction();
    		$sql = "UPDATE  ken_messe1
                      SET
                        sendday   = :sendday
                      WHERE id = :id AND messe_No = :messe_no";
    		$stmh = $this->pdo->prepare($sql);
    		$stmh->bindValue(':sendday',   $sendday,   PDO::PARAM_STR );
    		$stmh->bindValue(':id',         $id,         PDO::PARAM_INT );
    		$stmh->bindValue(':messe_no',         $messe_no,         PDO::PARAM_INT );
    		$stmh->execute();
    		$this->pdo->commit();
    ...
    //----------------------------------------------------
    // ken_messe2のsendday入力処理
    //----------------------------------------------------
    public function input_sendday_to_ken_messe2($id,$messe_no,$day_tmp){
    	try {
    		$this->pdo->beginTransaction();
    		$sql = "UPDATE  ken_messe2
                      ...
                      WHERE id = :id AND messe_No = :messe_no";
    		$stmh = $this->pdo->prepare($sql);
    		$stmh->bindValue(':sendday',   $day_tmp,   PDO::PARAM_STR );
    		$stmh->bindValue(':id',         $id,         PDO::PARAM_INT );
    		$stmh->bindValue(':messe_no',   $messe_no,         PDO::PARAM_INT );
    		$stmh->execute();
    		$this->pdo->commit();
    ...
    //----------------------------------------------------
    // ken_messe3のsendday入力処理
    //----------------------------------------------------
    public function input_sendday_to_ken_messe3($id,$messe_no,$day_tmp){
    	try {
    		$this->pdo->beginTransaction();
    ...
    //----------------------------------------------------
    // idとmesse_Noをキーにしてscheから削除する
    //----------------------------------------------------
    public function delete_sche($id,$messe_No){
        try {
            $this->pdo->beginTransaction();
            $sql = "DELETE FROM sche WHERE id = :id AND messe_No = :messe_No";
            $stmh = $this->pdo->prepare($sql);
            $stmh->bindValue(':id', $id, PDO::PARAM_INT );
            $stmh->bindValue(':messe_No', $messe_No, PDO::PARAM_INT );
            $stmh->execute();
...
EOS;
    ...
    // ken_messe1のデータ取得処理
    //----------------------------------------------------
    public function get_kongetu_data($day){
    	$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
		k.set_sendday as set_sendday,
		k.sendday_option as sendday_option,
...
EOS;
    	...
    	try {
    	$stmh = $this->pdo->prepare($sql);
    	$stmh->bindValue(':set_sendday',  $day, PDO::PARAM_INT );
    	$stmh->execute();
    	// 検索件数を取得
    	// 検索結果を多次元配列で受け取る
    		$i=0;
    ...
    //----------------------------------------------------
    // 来月分のken_messe1のデータ取得処理
    //----------------------------------------------------
    public function get_raigetu_data($day,$timing){
    	$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
...
FROM
        ken_messe1 k
WHERE
        set_sendday <= :set_sendday AND send_timing = :send_timing;
...
EOS;
    	...
    	$stmh = $this->pdo->prepare($sql);
    	$stmh->bindValue(':set_sendday',  $day, PDO::PARAM_INT );
    	$stmh->bindValue(':send_timing',  $timing, PDO::PARAM_INT );
    	$stmh->execute();
        	// 検索件数を取得
    		// 検索結果を多次元配列で受け取る
    		$i=0;
    ...
    // ken_messe2のデータ取得処理
    //----------------------------------------------------
    public function get_ken_messe2_week($youbi){
    	$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
...
EOS;
    	...
    	try {
    	$stmh = $this->pdo->prepare($sql);
    	$stmh->bindValue(':set_sendday',  $youbi, PDO::PARAM_INT );
    	$stmh->execute();
        	// 検索件数を取得
    		// 検索結果を多次元配列で受け取る
    		$i=0;
    ...
    //----------------------------------------------------
    // 上記のget_ken_messe2_weekを実行した時、何列ヒットするかを返す(whileで使用する)
    //----------------------------------------------------
    public function count_ken_messe2_week($youbi){
    	$sql= <<<EOS
    		SELECT
		k.id as id
FROM
...
EOS;
    ...
    //----------------------------------------------------
    // ken_messe3の最終営業日の行のみ取得処理
    //----------------------------------------------------
    public function get_final_eigyoubi($day){
    	$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
...
EOS;
    	...
    	try {
    	$stmh = $this->pdo->prepare($sql);
    		$stmh->bindValue(':send_timing',  $day, PDO::PARAM_INT );
    	$stmh->execute();
        	// 検索件数を取得
    		// 検索結果を多次元配列で受け取る
    		$i=0;
...
//----------------------------------------------------
//ken_messe3のset_senddayが０の行を全て取得
//----------------------------------------------------
public function get_send_timing(){
$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
...
EOS;
	...
	try {
	$stmh = $this->pdo->prepare($sql);
	$stmh->bindValue(':set_sendday',  0, PDO::PARAM_INT );
    $stmh->execute();
    	// 検索件数を取得
		// 検索結果を多次元配列で受け取る
		$i=0;
	...
	//----------------------------------------------------
	//ken_messe3のset_senddayが１の行を全て取得(一応登録できないようにはしてあるが、set_senddayとsend_timingが両方とも１だと
	//日付を決定する計算式がループしてしまうため、両方とも１になってる行は取得しないようにした)
	//----------------------------------------------------
	public function get_send_timing_from_back(){
		$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
...
EOS;
		...
		try {
			$stmh = $this->pdo->prepare($sql);
			$stmh->bindValue(':set_sendday',  1, PDO::PARAM_INT );
			$stmh->bindValue(':send_timing',  2, PDO::PARAM_INT );
			$stmh->execute();
			// 検索件数を取得
			// 検索結果を多次元配列で受け取る
			$i=0;
	...
	//----------------------------------------------------
	// 会員情報をidで検索し、ヒットした場合、メールアドレスを返す。
	//----------------------------------------------------
	public function get_mail_address($id){
		$data = [];
		try {
			$sql= "SELECT * FROM member2 WHERE id = :id limit 1";
			$stmh = $this->pdo->prepare($sql);
...
EOS;
	...
	//----------------------------------------------------
	//ken_messe1の送信すべきテキストを取得
	//----------------------------------------------------
	public function get_send_mail_ken_messe1($day,$time){
		$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
		k.send_text as send_text
FROM
        ken_messe1 k
WHERE
        sendday = :sendday AND send_time = :send_time;
...
EOS;
		...
		try {
				$stmh = $this->pdo->prepare($sql);
				$stmh->bindValue(':sendday',  $day, PDO::PARAM_STR );
				$stmh->bindValue(':send_time',  $time, PDO::PARAM_INT );
				$stmh->execute();
				// 検索件数を取得
		// 検索結果を多次元配列で受け取る
		$i=0;
	...
	//----------------------------------------------------
	//ken_messe2の送信すべきテキストを取得
	//----------------------------------------------------
	public function get_send_mail_ken_messe2($day,$time){
		$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
...
EOS;
	...
	//----------------------------------------------------
	//ken_messe3の送信すべきテキストを取得
	//----------------------------------------------------
	public function get_send_mail_ken_messe3($day,$time){
		$sql = <<<EOS
SELECT
		k.id as id,
		k.messe_No as messe_No,
