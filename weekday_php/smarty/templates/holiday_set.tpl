...
<FORM {$form.attributes}>
	...
	<BR></CENTER>{if ($day)}<div style="background:#F0FFFF;border-radius: 10px;">{/if}
	<BR>
    {if ($day)}
    <div class="form-group">
    <B>{$form.day.label}</B>{$form.day.html}<BR>
    </div>
    {/if}
    {if ($option) }
    <div class="form-group">
    <B>{$form.option.label}</B>{$form.option.html}<BR>
    </div>
    {/if}
    {if ($timing) }
     <div class="form-group">
    <B>{$form.timing.label}</B>{$form.timing.html}<BR>
    </div>
    {/if}
    {if ($time) }
    <div class="form-group">
