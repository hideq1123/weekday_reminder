...
<div class="form-group">
    <label for="inp2">{$form.username.label}</label><BR>
{if $form.username.error }{* エラーメッセージがあるときの処理 *}
<font size="2" color="red">{$form.username.error}</font><BR>{* エラーメッセージ *}
{/if}

{*
{if $form.username.html|mb_strpos:'outlook.com' !== FALSE}
<font size="1" color="red">マイクロソフト系のメールアドレスは迷惑メールへ振り分けられやすい為、ご注意願います。</font><BR>
{/if}
*}

{$form.username.html}<font size="2" color="red">{$message3}</font>{* 入力欄 *}
</div>


	      <div class="form-group"><label for="inp2">{$form.password.label}</label>
		{if $coution}
		<font size="1">(変更するしないに関わらず、ここに入力したものが登録されます。)</font><BR>
		{/if}
                {if $form.password.error }
                  <font size="1" color="red">{$form.password.error}</font><BR>
                {/if}
                {$form.password.html}{* 入力欄 *}</div>

<BR>
<CENTER>
            {if ( $form.submit2.value != "" ) }
              {$form.submit2.html}{* 戻るボタン(値がある場合) *}
            {else}
            {$form.reset.html}{* 取り消しボタン(値がない場合) *}</button>
            {/if}
            {$form.submit.html}</button>
</CENTER>
